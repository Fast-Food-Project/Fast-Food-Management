...
import AddOrdering from "./AddOrdering";
import DetailOrdering from "./DetailOrdering";
import ReportInvoiceList from "./ReportInvoiceList";
import EditOrdering from "./EditOrdering";
...
const StoresOrderingTableList = () => {
  ...
  const [isEditInvoiceOverlayOpen, setIsEditInvoiceOverlayOpen] =
    useState(false);
  const [isAddInvoiceOverlayOpen, setIsAddInvoiceOverlayOpen] = useState(false);
  ...
  const handleEdit = (id: string) => {
    router.push(`/invoice/store-ordering/edit/${id}`);
  };

  const handleAdd = () => {
    setIsAddInvoiceOverlayOpen(!isAddInvoiceOverlayOpen);
  };
  ...
  return (
    <div className="w-full flex flex-col gap-2">
      ...
      <AddOrdering
        isAddInvoiceOverlayOpen={isAddInvoiceOverlayOpen}
        setIsAddInvoiceOverlayOpen={setIsAddInvoiceOverlayOpen}
      />
    </div>
  );
};

export default StoresOrderingTableList;
